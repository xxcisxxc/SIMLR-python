...
QuadTree* buildQuadTree(double *Y, double *weights, int n) {
    ...
    for (i=0;i<n;i++) {
        for (d=0;d<2;d++) {
            minPos[d] = MIN(Y[d+i*2], minPos[d]);
            maxPos[d] = MAX(Y[d+i*2], maxPos[d]);
        }
    }
    for (d=0;d<2;d++) {
...
void dumpTree(QuadTree *tree, int depth) {
    ...
    if (tree->node != NULL)
        printf("node=(%d,%.2f), ", tree->node->id, tree->node->weight);
    else
        printf("node=NULL, ");
    printf("position=(%.2f,%.2f), ", tree->position[0], tree->position[1]);
    printf("weight=%f, ", tree->weight);
    printf("coef=%f, ", tree->coef);
    printf("boundary=(%.2f,%.2f,%.2f,%.2f), ", tree->minPos[0], tree->minPos[1], tree->maxPos[0], tree->maxPos[1]);
    printf("childCount=%d, ", tree->childCount);
    printf("\n");
    for (i=0;i<tree->childrenLength;i++) {
        if (tree->children[i]!=NULL)
